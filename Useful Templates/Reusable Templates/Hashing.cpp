...
/*
Polynomial rolling hash for strings. Supports O(1) substring hash queries after O(n) preprocessing.
To reduce the probability of collisions, double hashing is used (two different primes and moduli).
Randomly select prime indices for hashing to further reduce collision chances in competitive programming contexts.
*/
...
// Double hashing (to reduce the probability of collisions)
...
void solve() {
    // Example usage of string_Hash

    // Using single hashing
    string s = "hello";
    int idx1 = (rand() % 36) + 36; // Randomly select a prime index for hashing
    string_Hash hash(s);
    cout << "Hash of the full string: " << hash.fullStringHash() << '\n';
    cout << "Hash of substring 'ell': " << hash.substringHash(1, 3) << '\n';
    ...
    string_Hash double_hash(text, idx1, idx2);
    cout << "Double hash of the full string: " << double_hash.fullStringHash() << '\n';
    cout << "Double hash of substring 'ell': " << double_hash.substringHash(1, 3) << '\n';
}
