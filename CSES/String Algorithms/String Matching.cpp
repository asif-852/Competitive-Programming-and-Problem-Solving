...
void solve() {
    ...
    string text, pattern;
    cin >> text >> pattern;
    string_Hash H1(text, idx1, idx2), H2(pattern, idx1, idx2);
    int len = pattern.size();
    auto pattern_hash = H2.fullStringHash();
    int ans = 0;
    for(int i = 0; i + len - 1 < text.size(); i++) {
        ans += (H1.substringHash(i, i + len - 1) == pattern_hash);
    }
    cout << ans << '\n';
}
